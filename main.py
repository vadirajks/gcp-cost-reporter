...
print("✅ Starting GCP Cost Alerting Script (PrettyTable Version)")
...
def format_diff(last_month_total, this_month_total, days_so_far, last_month_days):
    try:
        if abs(last_month_total) < 0.01 or days_so_far == 0:
            if this_month_total > 1:
                return "(↑∞%)🔴", float('inf')
            else:
                return "", 0.0
        expected_so_far = (last_month_total / last_month_days) * days_so_far
        if abs(expected_so_far) < 0.01:
            return "(↑∞%)🔴" if this_month_total > 1 else "", 0.0
...
def format_daily_diff(average_cost, current_cost):
    if average_cost == 0:
        return "(↑∞%)🔴" if current_cost > 0 else ""
    percent_diff = ((current_cost / average_cost) - 1) * 100
    if percent_diff > 1: return f"(↑{abs(percent_diff):.0f}%)🔴"
    elif percent_diff < -1: return f"(↓{abs(percent_diff):.0f}%)🟢"
    else: return "(→0%)🟢"
...
def generate_service_summary_table(df_last: pd.DataFrame, df_this: pd.DataFrame, days_so_far: int, last_month_days: int, recent_days_to_display: list, seven_days_for_avg: list):
    ...
    display_day_strs = [d.strftime('%Y-%m-%d') for d in recent_days_to_display]
    avg_date_strs = [d.strftime('%Y-%m-%d') for d in seven_days_for_avg]
    headers = ["#", "Service", "Last Month", "This Month ( %diff )", "7 Day Avg"] + display_day_strs
    ...
    for i, service in enumerate(all_service_names, start=1):
        ...
        total_for_avg = df_this[(df_this['service_description'] == service) & (df_this['usage_date'].isin(avg_date_strs))]['subtotal'].sum()
        seven_day_avg = total_for_avg / 7
        daily_costs_raw = [df_this[(df_this['service_description'] == service) & (df_this['usage_date'] == day_str)]['subtotal'].sum() for day_str in display_day_strs]
        daily_diff_str = format_daily_diff(seven_day_avg, daily_costs_raw[-1])
        display_vals = [clean_currency(cost) for cost in daily_costs_raw]
        display_vals[-1] = f"{display_vals[-1]}{daily_diff_str}"

        table.add_row([i, service, clean_currency(last), f"{clean_currency(curr)}{diff_str}", clean_currency(seven_day_avg), *display_vals])
    ...
    total_for_avg_grand = df_this[df_this['usage_date'].isin(avg_date_strs)]['subtotal'].sum()
    seven_day_avg_grand = total_for_avg_grand / 7
    total_daily_grand_raw = [df_this[df_this['usage_date'] == day_str]['subtotal'].sum() for day_str in display_day_strs]
    daily_diff_grand_str = format_daily_diff(seven_day_avg_grand, total_daily_grand_raw[-1])
    total_display_vals = [clean_currency(cost) for cost in total_daily_grand_raw]
    total_display_vals[-1] = f"{total_display_vals[-1]}{daily_diff_grand_str}"
    
    table.add_row(['---'] * len(headers))
    table.add_row(["", "Total", clean_currency(grand_total_last), f"{clean_currency(grand_total_this)}{total_diff_str}", clean_currency(seven_day_avg_grand), *total_display_vals])
    
    return table.get_string()

def generate_sku_breakdown_table(service_id: str, service_name: str, df_last: pd.DataFrame, df_this: pd.DataFrame, days_so_far: int, last_month_days: int, recent_days_to_display: list, seven_days_for_avg: list):
    ...
    if service_name == "Compute Engine":
        def categorize_sku(sku_description):
            ...
            if 'local ssd' in sku_lower or 'local storage' in sku_lower: return 'Storage: Local SSD/Storage (Committed Use)' if 'commitment' in sku_lower else 'Storage: Local SSD/Storage (On-Demand)'
            ...
            if 'storage pd' in sku_lower or 'persistent disk' in sku_lower: return 'Storage: PD Snapshots' if 'snapshot' in sku_lower else 'Storage: Persistent Disk'
        ...
        category_order = ['VM Core Hours (On-Demand)', 'VM Core Hours (Committed Use)', 'VM Core Hours (Spot/Preemptible)', 'VM RAM (On-Demand)', 'VM RAM (Committed Use)', 'VM RAM (Spot/Preemptible)', 'Storage: Local SSD/Storage (On-Demand)', 'Storage: Local SSD/Storage (Committed Use)', 'Storage: Persistent Disk', 'Storage: PD Snapshots', 'Network: Data Transfer', 'Network: IP Addresses', 'Network: Cloud Router', 'Licenses', 'Other']
    ...
    display_day_strs = [d.strftime('%Y-%m-%d') for d in recent_days_to_display]
    avg_date_strs = [d.strftime('%Y-%m-%d') for d in seven_days_for_avg]
    headers = ["#", header_name, "Last Month", "This Month ( %diff )", "7 Day Avg"] + display_day_strs
    ...
    for i, item in enumerate(all_items, start=1):
        ...
        if last <= 1 and curr <= 1: continue
        diff_str, _ = format_diff(last, curr, days_so_far, last_month_days)
        total_for_avg = df_this_svc[(df_this_svc[grouping_key] == item) & (df_this_svc['usage_date'].isin(avg_date_strs))]['subtotal'].sum()
        seven_day_avg = total_for_avg / 7
        daily_costs_raw = [df_this_svc[(df_this_svc[grouping_key] == item) & (df_this_svc['usage_date'] == day_str)]['subtotal'].sum() for day_str in display_day_strs]
        daily_diff_str = format_daily_diff(seven_day_avg, daily_costs_raw[-1])
        display_vals = [clean_currency(cost) for cost in daily_costs_raw]
        display_vals[-1] = f"{display_vals[-1]}{daily_diff_str}"
        
        table.add_row([i, item, clean_currency(last), f"{clean_currency(curr)}{diff_str}", clean_currency(seven_day_avg), *display_vals])

    if not table.rows: return ""

    grand_total_last, grand_total_this = df_last_svc['subtotal'].sum(), df_this_svc['subtotal'].sum()
    total_diff_str, _ = format_diff(grand_total_last, grand_total_this, days_so_far, last_month_days)
    total_for_avg_grand = df_this_svc[df_this_svc['usage_date'].isin(avg_date_strs)]['subtotal'].sum()
    seven_day_avg_grand = total_for_avg_grand / 7
    total_daily_grand_raw = [df_this_svc[df_this_svc['usage_date'] == day_str]['subtotal'].sum() for day_str in display_day_strs]
    daily_diff_grand_str = format_daily_diff(seven_day_avg_grand, total_daily_grand_raw[-1])
    total_display_vals = [clean_currency(cost) for cost in total_daily_grand_raw]
    total_display_vals[-1] = f"{total_display_vals[-1]}{daily_diff_grand_str}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"❌ FATAL: Configuration file not found at '{args.config}'"); sys.exit(1)
    except Exception as e:
        print(f"❌ FATAL: Error parsing YAML configuration file: {e}"); sys.exit(1)
    ...
    if not all([BIGQUERY_BILLING_TABLE, BACKUP_DIR, DEFAULT_CHANNEL_ID, PROJECTS_TO_MONITOR]):
        print("❌ FATAL: One or more required keys are missing from the config file."); sys.exit(1)
    now = datetime.utcnow().date()
    today_str = now.strftime('%Y-%m-%d')
    first_of_this_month = now.replace(day=1)
    last_day_of_last_month = first_of_this_month - timedelta(days=1)
    first_of_last_month = last_day_of_last_month.replace(day=1)
    end_of_this_month_days = calendar.monthrange(now.year, now.month)[1]
    last_month_days = calendar.monthrange(first_of_last_month.year, first_of_last_month.month)[1]
    days_so_far = now.day
    recent_days_to_display = sorted([now - timedelta(days=i) for i in range(3, 0, -1)])
    if recent_days_to_display:
        last_displayed_day = recent_days_to_display[-1]
        seven_days_for_avg = [last_displayed_day - timedelta(days=i) for i in range(7, 0, -1)]
    else: seven_days_for_avg = []
    ...
    """
    for period, file_path, start_date, end_date in [("This Month", this_month_file, first_of_this_month.isoformat(), (now + timedelta(days=1)).isoformat()), ("Last Month", last_month_file, first_of_last_month.isoformat(), first_of_this_month.isoformat())]:
        df_name = f"df_{period.lower().replace(' ', '_')}_full"
        
        is_last_month_and_early = (period == "Last Month" and days_so_far <= 7)
        use_cache = os.path.exists(file_path) and not args.force_refresh and not is_last_month_and_early
        ...
        if use_cache:
            ...
            elif is_last_month_and_early:
                print(f"DATA REFRESH: Re-fetching data for Last Month within the first 7 days of the month to ensure data is final.")
            else:
    ...
    for project in PROJECTS_TO_MONITOR:
        ...
        if this_month_total < 1 and last_month_total < 1:
            ...
            continue
        forecast = (this_month_total / days_so_far) * end_of_this_month_days if days_so_far > 0 else 0
        diff_str, _ = format_diff(last_month_total, this_month_total, days_so_far, last_month_days)
        service_diff_report = generate_service_summary_table(df_last_project, df_this_project, days_so_far, last_month_days, recent_days_to_display, seven_days_for_avg)
        slack_message = [f"*GCP Cost Summary: {project_name}*", f"*Project ID:* `{project_id}`", "", "*Overall Cost:*", f"  • *Last Month Total:* `{clean_currency(last_month_total)}`", f"  • *This Month (so far):* `{clean_currency(this_month_total)}`{diff_str}", f"  • *Forecast (end of month):* `{clean_currency(forecast)}`", "", "*Cost Breakdown by Service:*", "```", service_diff_report, "```"]
        ...
        if not main_message_ts:
            ...
            continue
        last_month_services = df_last_project.groupby(['service_id', 'service_description'])['subtotal'].sum()
        this_month_services = df_this_project.groupby(['service_id', 'service_description'])['subtotal'].sum()
        all_services_indices = set(last_month_services.index) | set(this_month_services.index)
        def sort_key(service_index):
            _, service_name = service_index
            cost_this_month = this_month_services.get(service_index, 0)
            is_not_bigquery = 0 if 'bigquery' in service_name.lower() else 1
            return (is_not_bigquery, -cost_this_month)
        sorted_services_indices = sorted(all_services_indices, key=sort_key)
        for service_id, service_name in sorted_services_indices:
            cost_this_month = this_month_services.get((service_id, service_name), 0)
            cost_last_month = last_month_services.get((service_id, service_name), 0)
            if cost_this_month > 1 or cost_last_month > 1:
                print(f"    - Generating SKU breakdown for '{service_name}' (Current Cost: {clean_currency(cost_this_month)})...")
                table = generate_sku_breakdown_table(service_id, service_name, df_last_project, df_this_project, days_so_far, last_month_days, recent_days_to_display, seven_days_for_avg)
                if table.strip():
